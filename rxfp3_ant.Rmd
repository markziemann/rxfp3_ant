...
legend("bottomleft", inset=.02, title="tissue",
   levels(as.factor(ss$BrainRegion)) , fill=unique(cols),  cex=1.2)


legend("left", inset=.02, title="treatment",
   levels(as.factor(ss$Treatment)) , pch=unique(pchs),  cex=1.2)
...
mymds <- function(de,name,myss,mx) {
  ...
  legend("topright", inset=.02, title="treatment",
    levels(as.factor(ss$Treatment)) , pch=19, col=unique(csc),  cex=1.4)
}
